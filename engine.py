...
class Engine(object):
    def __init__(self, state={}):
        ...
        if self._state('train_workers') is None:
            self.state['train_workers'] = 2
        if self._state('test_workers') is None:
            self.state['test_workers'] = 2
    ...
    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        if display:
            if training:
                ...
                print('Epoch: [{0}]\t'
                      'Loss {loss:.4f}\t'
                      'mAP {map:.3f}'.format(self.state['epoch'], loss=loss.item(), map=map))
            else:
                #print(self.state['ap_meter'].value())

                print('Test: \t Loss {loss:.4f}\t  mAP {map:.3f}'.format(loss=loss.item(), map=map))
